...
%         pos2 = [dom2(1), dom2(2)];
%         vect = pos2-pos1;
%         dist = sqrt(vect(1)^2 + vect(2)^2)*0.19;
%         display(['Distance betwen Domino x ', int2str(pos1(1)), ' y ', int2str(pos1(2)), ' and Domino x ', int2str(pos2(1)), ' y ', int2str(pos2(2)), ': ', int2str(dist)]);
%     end
% end

%% Tracking
x = input('Press enter to begin tracking: ');
...
for n = 1:a(1)
  ...
  point2_x = B(2)+0.5*width*cos(B(4))-0.5*height*sin(B(4));
  point2_y = B(3)-0.5*width*sin(B(4))-0.5*height*cos(B(4)); 
  point3_x = B(2)+0.5*width*cos(B(4))+0.5*height*sin(B(4));
  point3_y = B(3)-0.5*width*sin(B(4))+0.5*height*cos(B(4));
  point4_x = B(2)-0.5*width*cos(B(4))+0.5*height*sin(B(4));
  point4_y = B(3)+0.5*width*sin(B(4))+0.5*height*cos(B(4));

  x1 = [point1_x, point2_x, point3_x, point4_x, point1_x];
  y1 = [point1_y, point2_y, point3_y, point4_y, point1_y];
  
  RectangleInfo = [RectangleInfo; x1, y1];
end

locations = [NoDuplicates(:, 2:3); NoDuplicatesC];

a = size(RectangleInfo);
b = size(locations);
newLocations = [];
for i = 1:a(1)
    for j = 1:b(1)
        in = inpolygon(locations(j, 1), locations(j, 2), RectangleInfo(i, 1:5), RectangleInfo(i, 6:10));
        if(in)
            newLocations = [newLocations; locations(j, :)];
        end
    end
end
...
%Create tracker objects for corners and circles
tracker = vision.PointTracker('MaxBidirectionalError', 5, 'BlockSize', [51,51]);
initialize(tracker, locations, gamma);
points_circles = step(tracker, gamma);
previousPoints = points_circles;
out_circles = insertMarker(gamma, points_circles, 'o');
setPoints(tracker, points_circles);

set(h, 'Cdata', out_circles);
drawnow;

totalText = [];

%While loop to process each image
while ~isempty(findall(0,'Type','Figure'))
    ...
    %Delete points/line from previous frame
    b = findobj('type','line');
    delete(b)
    
    %Get new points and validity from trackers
    points_circles = step(tracker, gamma);
    
    %Construct markers for the tracked points
    out_circles = insertMarker(gamma, points_circles, 'o');
    
    %Output points on video frame
    set(h, 'Cdata', gamma);
    set(h, 'Cdata', out_circles);
    ...
    %Recalculate points that may have been lost
    setPoints(tracker, points_circles);
    
    time = toc;
    
    vect = points_circles - previousPoints;
    
    a = size(vect);
    dist = [];
    for i = 1:a(1)
        dist = [dist; sqrt( (vect(i, 1))^2 + (vect(i, 2))^2 )];
    end
    
    b = findobj('type','text');
    delete(b)
    
    speed = (dist*mmpx/time);

    for i = 1:a(1)
        if(speed(i) > 3)
            text(double(points_circles(i,1)), double(points_circles(i,2)), ['\leftarrow Speed of this point : ', int2str(speed(i))] );
        end
    end
    
    totalText = [totalText; speed'];
